...
export function ChangePasswordClient() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setError(payload?.error || "No fue posible cambiar la contraseña");
      return;
    }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[var(--background)] px-4 py-10">
      <div className="w-full max-w-[430px] rounded-[20px] bg-white p-6 card-shadow sm:p-8">
        ...
        <h1 className="mt-2 text-[24px] font-semibold text-[var(--foreground)]">Cambiar contraseña temporal</h1>
        <p className="mt-2 text-[13px] leading-6 text-[var(--brand-secondary)]">Debes definir una contraseña nueva antes de usar el portal.</p>
